...
\caption{\inline|ServerComTask|}
\label{fig:servercomtask}
\end{figure}
\inline{ServerComTask} is a generic class, it declares a type which the abstract method \inline{ParseJson()} must create from the JSON it is supplied with.
...
\item[Lines 5-6] When the request is done we display the recipe search results in the \inline{this.RecipeSearchFragment}, and hide the progress circle.

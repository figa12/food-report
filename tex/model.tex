...
We have two different representation of a recipe, \inline{Recipe} and \inline{IntermediateRecipe}. The \inline{IntermediateRecipe} is a small subset of a recipe which is used to display search results. When the user clicks the search results, then the rest of a \inline{Recipe} is downloaded from the server and displayed to the user.
...
The reason why the attributes from \inline{Unit} is not modelled inside \inline{IngredientQuantity} is because the model resembles our relational database as much as possible. The \inline{Ingredient} class could also have been modelled inside \inline{IngredientQuantity}, but \inline{Ingredient} is used elsewhere. To use ingredient search we need to know the ingredients in order to use them for the search.
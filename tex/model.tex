...
The use of ingredients are easier understood by looking at \autoref{fig:ingredients}. If an ingredient can be exchanged by another ingredient it can be clicked, which will reveal the different ingredients that can be used. It can be seen that \textit{2 dl powdered sugar} is exchangeable with \textit{2 dl sugar}.

The reason why the attributes from \inline{Unit} are not modelled inside \linebreak\inline{IngredientQuantity} is because the model resembles our relational database as much as possible. The \inline{Ingredient} class could also have been modelled inside \inline{IngredientQuantity}, but \inline{Ingredient} is used when searching by ingredients.

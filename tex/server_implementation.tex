...
As seen in \autoref{lst:getIngredients} the method \inline{echoJson} is called. This method will encode the result into a JSON formatted object and then print it. An example of result from a request to the server could look like the following:
...
\item[favourites] This request takes two required parameters and four optional parameters. The two required parameters are: \textit{action} and \textit{hash}. The four optional parameters are: \textit{recipeid}, \textit{limit}, \textit{offset}, and \textit{lang}.\\
  ...
  \textbf{status:} Requires \textit{recipeid} must be set. This request will return a boolean value indicating whether the user already has the given recipe favourited or not.\\\\
  \textbf{add:} Requires \textit{recipeid} must be set. This request will set the given recipe as a favourite of the user.\\\\
  \textbf{remove:} Requires \textit{recipeid} must be set. This request will remove the given recipe from user's favourites.\\\\
  \textbf{get:} Requires \textit{limit}, and \textit{offset} must be set. This request will return a list of all the user's favourited recipes\\\\

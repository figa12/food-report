...
If the user did not grant access to the application the routine starts all over. When the user is connected \textit{onConnected()} is called, this method simply updates the UI and downloads information from the user's Google+. \todo{Should there be code snippets in this section?}
...
It might seem overrated that we send the hashed email every time and not just an user id, but the mobile device is not required to have an internet connection\todo{Internet or connection to the server?} to be able to sign in using Google+. 

...
\item[Line 6] The result is limited and offset according to the applications needs i.e. how far the user has scrolled through the result.
\item[Lines 1-3] The ID of each recipe, and the number of covered mandatory and optional ingredients are returned. To calculate this, we first replace the cover count with \inline{NULL} if the count is 0. Since \inline{NULL} values are ignored by the \inline{COUNT} function, we will get the number of uniquely covered exchangeables.\todo{improve}
\end{description}
...
\item[Lines 1-3] The ID of each recipe, and the number of lacking mandatory and optional ingredients are returned. To calculate this, we first replace the cover count with 0 if the count is \inline{NULL} due to the \inline{RIGHT JOIN} in \inline{$lackingQuery}. We then substract the mandatory cover count from the total number of mandatories to get the number of lacking exchangeables. WE do the same to get the number of lacking optional exchangeables.\todo{improve, omdøb til nonCovered?}
\end{description}
...
\item[Lines 8-9] The result is ordered as defined in \secref{sec:design_search}. In descending order of precedence: most mandatory matching, least mandatory lacking, most optional matching, least optional lacking. \todo{mandatory not covered?}

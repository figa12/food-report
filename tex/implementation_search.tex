...
\item[Lines 2-3] When you use full text search in the default mode, \inline{NATURAL LANGUAGE MODE}, it calculates a score which reflect the relevance of the match. A high relevance could indicate that the matched query represent a big part of the text. The scores for when the user defined text is matched to the recipes' titles and descriptions are calculated using \inline{NATURAL LANGUAGE MODE}. These values are stored in respectively \inline{title_score} and \inline{desc_score}.
